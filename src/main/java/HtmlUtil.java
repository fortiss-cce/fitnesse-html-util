...
public class HtmlUtil {
    ...
    public static String testableHtml(PageData pageData, boolean includeSuiteSetup) throws Exception {
        ...
        if (pageData.hasAttribute("Test")) {
            if (includeSuiteSetup) { // @TODO: Currently untested!
                testableHtml_WithSetup(pageData, buffer, wikiPage);
            } else {
                testableHtml_WithoutSetup(pageData, buffer, wikiPage);
            }
        } else {
    ...
    private static void testableHtml_WithSetup(PageData pageData, StringBuffer buffer, WikiPage wikiPage) throws Exception {
    ...
    private static void testableHtml_WithoutSetup(PageData pageData, StringBuffer buffer, WikiPage wikiPage) throws Exception {
        WikiPage setup = PageCrawlerImpl.getInheritedPage("SetUp", wikiPage);
        if (setup != null) {
            WikiPagePath setupPath = wikiPage.getPageCrawler().getFullPath(setup);
            String setupPathName = PathParser.render(setupPath);
